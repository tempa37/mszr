...
void StartTask02(void *argument)
{
  ...
  for(;;)
  {
    ...
    if(adc_ready == 1)
    {
      ...
      REGISTERS[1] = rms * 0.019922;  // REGISTERS[1] = ((((rms / 4096) * 3.3) * 3) / (121.1775) * 1000);
    }
...
//------------------------------------------------------------------------------
          case STATE_CREATE_SOCKET:
              ...
              err_t bind_result = netconn_bind(nc, &local_ip, 502);
...
const char * SAVE_CGI_Handler(int iIndex, int iNumParams, char *pcParam[], char *pcValue[])
{
    ...
    if(pinaccept)
    {
      ...
      if(paritiy_flag != 0)
      {
        ...
        WriteFlash(RS485PARITIY, rs485paritiy);
        memset(output, 0, sizeof(output));
        paritiy_flag = 0;   
      ...
      //------------in this place transmith /notion page with ssi tags (new value)
      restart = 1;
      return 0;
    }
  return 0;
...
void EXTI9_5_IRQHandler(void)  //------PA6----------
{
...
void EXTI15_10_IRQHandler(void) //-----PA12-----------
{
